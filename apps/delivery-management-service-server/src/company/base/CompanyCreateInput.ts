/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AddressWhereUniqueInput } from "../../address/base/AddressWhereUniqueInput";
import { ValidateNested, IsOptional, IsString } from "class-validator";
import { Type } from "class-transformer";
import { DeliveryRequestCreateNestedManyWithoutCompaniesInput } from "./DeliveryRequestCreateNestedManyWithoutCompaniesInput";
import { RiderCreateNestedManyWithoutCompaniesInput } from "./RiderCreateNestedManyWithoutCompaniesInput";

@InputType()
class CompanyCreateInput {
  @ApiProperty({
    required: false,
    type: () => AddressWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AddressWhereUniqueInput)
  @IsOptional()
  @Field(() => AddressWhereUniqueInput, {
    nullable: true,
  })
  address?: AddressWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: () => DeliveryRequestCreateNestedManyWithoutCompaniesInput,
  })
  @ValidateNested()
  @Type(() => DeliveryRequestCreateNestedManyWithoutCompaniesInput)
  @IsOptional()
  @Field(() => DeliveryRequestCreateNestedManyWithoutCompaniesInput, {
    nullable: true,
  })
  deliveryRequests?: DeliveryRequestCreateNestedManyWithoutCompaniesInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => RiderCreateNestedManyWithoutCompaniesInput,
  })
  @ValidateNested()
  @Type(() => RiderCreateNestedManyWithoutCompaniesInput)
  @IsOptional()
  @Field(() => RiderCreateNestedManyWithoutCompaniesInput, {
    nullable: true,
  })
  riders?: RiderCreateNestedManyWithoutCompaniesInput;
}

export { CompanyCreateInput as CompanyCreateInput };
